...
```mermaid
graph TD
    A["Users (Domain-Joined AD Accounts)"] -->|HTTPS + Negotiate (SSO)| B[Blazor Server UI (IIS)]
    B -->|REST (OpenAPI Contract)| C[ASP.NET Core REST API]
    C -->|EF Core| D[SQL Server 2022]
    C --> E["Local AI Agent (Llama / Ollama)"]
    C --> K["Central Logs + Traces (Serilog / OpenTelemetry)"]
    B --> F["Azure DevOps Pipeline Integration (YAML)"]
    F --> G["Test Environment"]
    F --> H["Production Environment"]
    G -->|Deploy (IIS; containers optional)| I["IIS on Windows Server 2022"]
    H -->|Deploy (IIS; containers optional)| J["IIS on Windows Server 2022"]
```
